...
#Open the list of runs created with GenerateRunList.py- ALL RUNS ARE GOIN IN HERE
outpath = sys.path[0]
outfile = outpath + "\\nemoruns.txt"
file_of_runs = open(outfile, 'r')

#Create an output csv file
outcsv = outpath + "\\DeleteMe.csv"
...
header_list = ["RunID","InstrumentID","RunDate","NumberOfCyclesR1","NumberofCyclesR2","FlowCellInDate?",
               ...
               "ProportionOverQ30ExceedsThresholdRead2","SlopeOfLineOfBestFitRead1","ThresholdSlopeR1",
               "SlopeOfLineOfBestFitRead1ExceedsThreshold?", "SlopeofLOBFR1_1","SlopeOfLOBFR1_2", "R1_1overThresh?","R1_2overthresh",
               "SlopeOfLineOfBestFitRead2","ThresholdSlopeR2","SlopeOfLineOfBestFitRead2ExceedsThreshold?","ProportionOfUndeterminedReads?",
...
for run in file_of_runs:
    ...
    read_perc_threshold = 75 #Set the low threshold for reads passing filter
    ...
    outside_threshold_dens = {"2":(600,1300),"3":(800,1700)}
    ...
    if threshold_vals_dens == None or outside_threshold_vals_dens == None:
        ...
        #Checks for thresholding levels on cluster density- fixed 04/03/16 as was testing passing filter not raw values
        if (mean_dens < outside_threshold_vals_dens[0]):
            data_for_run.append("very low")
        elif (mean_dens > outside_threshold_vals_dens[1]):
            data_for_run.append("very high")
        elif (mean_dens < threshold_vals_dens[0]):
            #outside Illumina recommended thresholds
            data_for_run.append("low")
        elif (mean_dens > threshold_vals_dens[1]):
            #outside Illumina recommended thresholds
            data_for_run.append("high")
        else:
            data_for_run.append("OK")
    ...
    threshold_COV_cluster_density = 0.08 # (8%)
    ...
    threshold_med_cd = 80 #Is this a useful threshold?
    ...
    Q30_median_over_cycle_r1_xdata = (np.arange(1,(len(Q30_median_over_cycle_r1)+1))) # Want it to start at 1 and stop at length + 1
    ...
    optimal_vals_Q30_r1 = read_means.linearFit(Q30_median_over_cycle_r1_xdata,Q30_median_over_cycle_r1,p1=-0.05,p2=-0.05)
    linear_pred_r1 = ((optimal_vals_Q30_r1[0] * Q30_median_over_cycle_r1_xdata) + optimal_vals_Q30_r1[1])
    
    #Break read into two halves
    Q30_median_over_cycle_r1_xdata_1 = (Q30_median_over_cycle_r1_xdata[0:((len(Q30_median_over_cycle_r1_xdata)/2))])
    Q30_median_over_cycle_r1_xdata_2 = (Q30_median_over_cycle_r1_xdata[((len(Q30_median_over_cycle_r1_xdata)/2)):])
    Q30_median_over_cycle_r1_1 = (Q30_median_over_cycle_r1[0:((len(Q30_median_over_cycle_r1)/2))])
    Q30_median_over_cycle_r1_2 = (Q30_median_over_cycle_r1[((len(Q30_median_over_cycle_r1)/2)):])
    
    #Get gradient of both halves
    optimal_vals_Q30_r1_1 = read_means.linearFit(Q30_median_over_cycle_r1_xdata_1,Q30_median_over_cycle_r1_1,p1=-0.05,p2=-0.05)
    linear_pred_r1_1 =((optimal_vals_Q30_r1_1[0] * Q30_median_over_cycle_r1_xdata_1) + optimal_vals_Q30_r1_1[1])
    
    optimal_vals_Q30_r1_2 = read_means.linearFit(Q30_median_over_cycle_r1_xdata_2,Q30_median_over_cycle_r1_2,p1=-0.05,p2=-0.05)
    linear_pred_r1_2 = ((optimal_vals_Q30_r1_2[0] * Q30_median_over_cycle_r1_xdata_2) + optimal_vals_Q30_r1_2[1])

    #Work out the thresholding limits
    #For a greater range of options regarding cycle number
    # Modify these according to need
    start_x = 50
    stop_x = 300
    polynomial_order = 5
    std_threshold_set = 2
    threshold_slope_r1 = read_means.returnThresholdGrad(start_x,stop_x,"r1",polynomial_order,std_threshold_set,read1_length)
    
    #Fit an exponential?
    threshold_slope_r1 = read_means.returnThresholdGrad2(start_x,stop_x,"r1",polynomial_order,std_threshold_set,read1_length)
    
    threshold_slope_linear_pred_r1 = ((threshold_slope_r1 * Q30_median_over_cycle_r1_xdata) + optimal_vals_Q30_r1_2[1])
    ...
    data_for_run.append(threshold_slope_r1)
    
    if abs(slope_Q30_r1) > abs(threshold_slope_r1):
        data_for_run.append("yes")
    else:
        data_for_run.append("no")
    
    
    #Figure out if particular positions are not typical of their position in the read- IN THIS CHECK WILL NEED TO TYPICALLY EXCLUDE LAST CYCLE AS IT SUCKS
    #Get the standard deviation over the read
    #Upper threshold is useless as it often exceeds the physically possible- don't include
    std_threshold_Q30 = 2 #Set desired threshold outside of which are outliers
    read_1_Q30_std = np.std(Q30_median_over_cycle_r1)
    read_1_Q30_std_threshold_upper = linear_pred_r1 + (std_threshold_Q30 * read_1_Q30_std)
    read_1_Q30_std_threshold_lower = linear_pred_r1 - (std_threshold_Q30 * read_1_Q30_std)
    
    read_1_Q30_std_threshold_lower_1 = linear_pred_r1_1 - (std_threshold_Q30 * read_1_Q30_std)
    read_1_Q30_std_threshold_lower_2 = linear_pred_r1_2 - (std_threshold_Q30 * read_1_Q30_std)
    
    
    ####CHANGES MADE TO INSPECT THIS
    pl.figure(1)
    pl.plot(Q30_median_over_cycle_r1_xdata,Q30_median_over_cycle_r1)
    pl.plot(Q30_median_over_cycle_r1_xdata,linear_pred_r1)
    #pl.plot(Q30_median_over_cycle_r1_xdata,threshold_slope_linear_pred_r1)
    #pl.plot(Q30_median_over_cycle_r1_xdata,read_1_Q30_std_threshold_upper)
    pl.plot(Q30_median_over_cycle_r1_xdata,read_1_Q30_std_threshold_lower)
    pl.plot(Q30_median_over_cycle_r1_xdata_1,read_1_Q30_std_threshold_lower_1)
    pl.plot(Q30_median_over_cycle_r1_xdata_2,read_1_Q30_std_threshold_lower_2)    
    pl.plot(Q30_median_over_cycle_r1_xdata_1,linear_pred_r1_1)
    pl.plot(Q30_median_over_cycle_r1_xdata_2,linear_pred_r1_2)
    pl.show()
    ...
    if r2_start < (len(qual_metrics_arr_mod_int_sorted)):
        ...
        #This requires a function as input
        optimal_vals_Q30_r2 = read_means.linearFit(Q30_median_over_cycle_r2_xdata,Q30_median_over_cycle_r2,p1=-0.05,p2=-0.05)
        ...
        threshold_slope_r2 = read_means.returnThresholdGrad(start_x,stop_x,"r2",polynomial_order,std_threshold_set,read2_length)
        
        data_for_run.append(threshold_slope_r2)
        
        if abs(slope_Q30_r2) > abs(threshold_slope_r2):
            data_for_run.append("yes")
        else:
            data_for_run.append("no")
